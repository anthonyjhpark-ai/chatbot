...
console.log('🔄 시즌 데이터 업데이트: 2024-25 → 2025-26\n');
...
async function updateTo2526() {
  try {
    // 1. 2024-25 시즌 데이터 조회
    console.log('📊 2024-25 시즌 데이터 조회 중...');
    const oldStats = await supabaseRequest('GET', '/rest/v1/player_season_stats?season=eq.2024-25&select=id,player_id');
    ...
    // 2. 각 레코드를 2025-26으로 업데이트
    console.log('🔄 시즌을 2025-26으로 업데이트 중...');
    let updated = 0;
    ...
    // 3. 결과 확인
    const newStats = await supabaseRequest('GET', '/rest/v1/player_season_stats?season=eq.2025-26&select=*,nba_players(name,team)&order=points.desc&limit=10');
